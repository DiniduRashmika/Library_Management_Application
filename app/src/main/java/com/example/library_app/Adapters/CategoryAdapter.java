...
public class CategoryAdapter extends RecyclerView.Adapter<CategoryAdapter.ViewHolder> {
   ...
   private ArrayList<CategoryDomain> items;
    ...
    public void onBindViewHolder(@NonNull CategoryAdapter.ViewHolder holder, int position) {
       ...
       int dramableResourceId=holder.itemView.getResources().getIdentifier(items.get(position).getPicPath()
               , "drmable",holder.itemView.getContext().getPackageName());

       Glide.with(holder.itemView.getContext())
               .load(dramableResourceId)
               .into(holder.picImg);
